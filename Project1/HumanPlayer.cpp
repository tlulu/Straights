...
HumanPlayer::PlayerException::PlayerException () {}
...
HumanPlayer::IllegalPlayException::IllegalPlayException () {};

HumanPlayer::IllegalPlayException::~IllegalPlayException () {};

std::string HumanPlayer::IllegalPlayException::message () const {
...
HumanPlayer::IllegalDiscardException::IllegalDiscardException () {};

HumanPlayer::IllegalDiscardException::~IllegalDiscardException () {};

std::string HumanPlayer::IllegalDiscardException::message () const {
...
// Play a card
void HumanPlayer::play (Card card, Board& board, const Hand& legalPlays ) {
	if (!legalPlays.hasCard( card )){
		throw IllegalPlayException ();		// Card is not part of legal play
	}

	Player::play (card, board, legalPlays);
}

// Discard a card
void HumanPlayer::discard (Card card, const Hand& legalPlays) {
	if ( legalPlays.size()!=0 ){
		throw IllegalDiscardException ();	// There is a legal play, cannnt discard
	}

	Player::discard (card, legalPlays);
}
